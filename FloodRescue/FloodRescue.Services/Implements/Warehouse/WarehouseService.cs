...
namespace FloodRescue.Services.Implements.Warehouse
{
    public class WarehouseService : IWarehouseService
    {
        ...
        {
            _logger.LogInformation("[WarehouseService] Request to create new Warehouse. Name: {WarehouseName}", request.Name);
        ...
        {
            _logger.LogInformation("[WarehouseService] Request to delete Warehouse ID: {WarehouseId}", warehouseId);
            WarehouseEntity? warehouse = await _unitOfWork.Warehouses.GetAsync(w => w.WarehouseID == warehouseId);
            int result = 0;
            if (warehouse == null)
            {
                _logger.LogWarning("[WarehouseService - Sql Server] Delete failed. Warehouse ID: {WarehouseId} not found.", warehouseId);
                return false;
            }
            if (warehouse?.IsDeleted == true)
            {
                _logger.LogInformation("[WarehouseService] Warehouse ID: {WarehouseId} was already deleted. No changes made.", warehouseId);
                return false;
            }
            ...
            {
                _logger.LogError("[WarehouseService - Sql Server] Unexpected error: Changes were not saved for Warehouse ID: {WarehouseId}", warehouseId);
                return false;
            }
        }
        public async Task<ShowWareHouseResponseDTO?> SearchWarehouseAsync(int id)
        {
            ...
            _logger.LogInformation("[WarehouseService] Searching for Warehouse with ID: {WarehouseID}", id);
            var cache = await _cacheService.GetAsync<ShowWareHouseResponseDTO>($"{WAREHOUSE_KEY_PREFIX}{id}");
            if (cache != null)
            {
                _logger.LogInformation("[WarehouseService - Redis] Found Warehouse in cache: {WarehouseID}", id);
                return cache;
            }
            ...
            if (warehouse != null)
            {
                ...
                _logger.LogInformation("[WarehouseService - Redis] Added Warehouse to cache: {WarehouseID}", id);
            }
            _logger.LogWarning("[WarehouseService - Sql Server] Warehouse with ID: {WarehouseId} not found in database.", id);
            return responseDTO;
        }

        public async Task<List<ShowWareHouseResponseDTO>> GetAllWarehousesAsync()
        {
            _logger.LogInformation("[WarehouseService] Getting all Warehouse");
            ...
            _logger.LogInformation("[WarehouseService - Sql Server] Retrieved {Count} warehouses from database", result.Count);

            await _cacheService.SetAsync(ALL_WAREHOUSES_KEY, result, TimeSpan.FromMinutes(5));

            _logger.LogInformation("[WarehouseService - Redis] Cached {Count} warehouses", result.Count);
        ...
        {
            _logger.LogInformation("[WarehouseService] Request to update Warehouse ID: {WarehouseId}", id);
            WarehouseEntity? _warehouse = await _unitOfWork.Warehouses.GetAsync(w => w.WarehouseID == id);

            if (_warehouse == null)
            {
                _logger.LogWarning("[WarehouseService - Sql Server] Update failed. Warehouse ID: {WarehouseId} not found.", id);
                return null;
            }
            ...
            _logger.LogInformation("[WarehouseService - Sql Server] Successfully updated Warehouse ID: {Id} changed name from '{OldName}' to '{NewName}'",
